  ...
  const columns: TableProps<DataType>["columns"] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Space size="middle">
            <Tag color="#87d068">{_.clickNum}</Tag>
  ...
  const onFinish: FormProps<FieldType>["onFinish"] = async (values) => {
    try {
      ...
      setLists((lists: any) => {
        const newLists = lists.map((item: any, index: number) => {
          // 统一修改像素
          item.pixels = values.pixels?.replace(/\s+/g, "");
          // 修改指定号信息
          if (index === values.key) {
            item.comment = values.comment?.replace(/\s+/g, "");
            item.lineName = values.lineName?.replace(/\s+/g, "");
          }
          return item;
        });
        return newLists;
...
export async function getServerSideProps(context: any) {
  ...
  try {
    // 发送请求：根据 id 获取问卷数据
    const res: any = await getQuestionByDomainName(id);
    if (res.code === 201) {
      console.log(res);
